...
int tiempo[] =     {10000,4000,5000,6000};

String menuSel[] = {"->Triturar","->Secuencia","",""};
String menuDes[] = {"Secuencia","","",""};
int pines[] =      {2,3,1};
...
void motor(){
  lcd_1.clear();
  lcd_1.print("moviendo motor");
               


  digitalWrite(6, HIGH); //led indicativo  
  digitalWrite(dirPin, HIGH); //configura pin para girar a sentido horario
  ...
  for (int x = 0; x < 4; x++){
    ...
    lcd_1.print ("Horario...");
    for (int i = 0; i < 400; i++) {  
      digitalWrite(stepPin, HIGH); //enciende pin de paso
    ...
    digitalWrite(dirPin, LOW);   //configura pin para girar a sentido anti-horario
    
    lcd_1.clear();
    lcd_1.setCursor(0,0);
    lcd_1.print("moviendo motor");
    lcd_1.setCursor(0,1);
    lcd_1.print ("Anti Horario...");
